// JetVasSpellExplosion.
// From Ultima Online (Older one)
class JetVasSpellExplosion expands VasCastSpell;
...
function SpellRUN()
{
	...
	if(DamagePoints < 10)
		DamagePoints = 10;
	CastMassage="You cast Explosion @ "$DamagePoints$" Powerlevel ";
	ThrowFireball();
	PostSpell();
}
...
local int ballspeed;
	...
	ball = Spawn(class'JetVasSpells.JetVasSpellExplosionProjectile',Caster,,FireLocation,pawn(Caster).ViewRotation);
	...
	self.PlaySound(Sound'CreaturesSnd.Mech.mechhit02', SLOT_None,,,, 1.0 + FRand()*0.4-0.2);
}
